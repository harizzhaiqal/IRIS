...
**`profiles` is now `public.profiles`.** Renamed throughout the schema, the RLS policies, the seed, the generated types, and `lib/queries/profiles.ts`. It sits alongside `auth.users` rather than replacing it: `public.profiles` is the staff directory that IRIS owns and `auth.users` is the credential store that Supabase Auth owns. The link between them was `public.profiles.id`; it is now `public.profiles.auth_user_id`, per the integer-keys entry above. Every reference to the auth table in the SQL is schema-qualified, so the two never collide. Earlier entries in this file that mention `profiles` describe decisions made before the rename and are left as written.
...
**The seed creates no scratch table.** The training generator staged its list of people in `public.seed_people`, created near the top of the seed and dropped at the bottom. Anything that runs the file other than start-to-finish in one go — a partial paste, an earlier statement failing, the SQL Editor splitting the script — leaves the loop reading a table that does not exist yet or no longer does. The list is now an inline `VALUES` join inside the loop itself, so there is nothing to create and nothing to drop. `verify-bundle.mjs` fails if any `DROP TABLE` appears in the bundle at all, since a scratch table always shows up as the file dropping something it created.

**`setup.sql` clears auth accounts that have no profile, rather than matching on an email domain.** The header deletes the demo accounts before reseeding, and when the roster moved to `@irs.com.my` that filter stopped matching the older `@irssoftware.test` rows — so they survived every rebuild. That is not untidiness: `auth.users` belongs to Supabase Auth and knows nothing about profiles, so an orphaned account can still sign in, and the app then finds no profile, redirects to login, and loops on a blank screen. The sweep now runs after the seed and removes anything without a matching profile, which needs no editing the next time the roster changes. `verify-bundle.mjs` plants an account under the old domain before applying the bundle and asserts it is gone.

## Naming and account rebuilds

**`public.users` is `public.profiles` again.** Two tables called `users` in one database — one in `public`, one in `auth` — is a trap every time a function body, a policy, or a query has to be read. `profiles` cannot be mistaken for Supabase Auth's table, which is exactly why it was named that originally. The reverse rename protected `auth.users` explicitly during the pass, since a blanket replace would have silently repointed the credential table.
