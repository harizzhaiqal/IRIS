import Link from "next/link";
import { redirect } from "next/navigation";
import { ArrowLeft } from "lucide-react";

import { Button } from "@/components/ui/button";
import { requireProfile } from "@/lib/auth";
import { isReadOnlyRole } from "@/lib/types";
import { RequestForm } from "./request-form";

export const metadata = { title: "New request — IRIS" };

export default async function NewRequestPage() {
  const profile = await requireProfile();

  // The CEO remains view-only; staff, HOD and HR/admin may create requests.
  if (isReadOnlyRole(profile.role)) redirect("/requests");

  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="space-y-3">
        <Button asChild variant="ghost" size="sm" className="-ml-2">
          <Link href="/requests">
            <ArrowLeft className="h-4 w-4" />
            Requests
          </Link>
        </Button>

        <div className="space-y-1">
          <h1 className="text-2xl font-semibold tracking-tight">New request</h1>
          <p className="text-sm text-muted-foreground">
            Ask for equipment, an office item, or support. Your request is saved
            directly to the request list.
          </p>
        </div>
      </div>

      <RequestForm userId={profile.id} />
    </div>
  );
}
